...
class StockpileService:
    ...
    async def process_remote_and_ingest(
        ...
        # Determine structural details from OCR result
        ...
        items = self._map_ocr_data_to_items(df, valid_keys)
    ...
    def _map_ocr_data_to_items(
        self, df: pd.DataFrame, valid_keys: set[Tuple[str, str]]
    ) -> List[Dict[str, Any]]:
        """Maps OCR DataFrame rows to a list of item dictionaries."""
        items = []
        for _, r in df.iterrows():
            cname = str(r.get("CodeName", "")).strip()
            iname = str(r.get("Name", "")).strip()

            if (cname, iname) not in valid_keys:
                continue
    ...
    async def get_requisition_comparison(
        ...
        # Handle Priority and Non-Priority items
    ...
    def _process_priority_requisition(
        ...
        for p in priority_list:
            ...
            # Fallback for missing availability
    ...
    def _process_non_priority_requisition(
        ...
        for cname in sorted(non_priority, key=lambda c: codename_to_name.get(c, c)):
            ...
            # Check Crated
            ship_total_crated = ship_map.get((cname, True), 0)
            if ship_total_crated > 0:
                comparison_data.append(
                    {
                        "Item": codename_to_name.get(cname, cname),
                        "Avail": self.get_qty_crates(ship_total_crated, qpc, per_crate),
                        "Need": 0,
                        "is_crated": True,
                    }
                )
            ...
            if is_recv_hub:
                ship_total_loose = ship_map.get((cname, False), 0)
                if ship_total_loose > 0:
                    comparison_data.append(
                        {
                            "Item": codename_to_name.get(cname, cname),
                            "Avail": self.get_qty_crates(
                                ship_total_loose, qpc, per_crate
                            ),
                            "Need": 0,
                            "is_crated": False,
                        }
    ...
    async def locate_item(
        ...
        processed_results = []
        for r in results:
            # Calculate distance using hex-math utility
