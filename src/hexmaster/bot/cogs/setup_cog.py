...
class SetupCog(commands.Cog):
    ...
    @app_commands.describe(
        faction="Your faction (Colonial or Warden)", shard="The shard you are playing on (Alpha, Bravo, or Charlie)"
    )
    ...
    async def configure(self, interaction: discord.Interaction, faction: str | None = None, shard: str | None = None):
                ...
                if priority_csv.exists():
                    ...
                    for _, row in df.iterrows():
                        await self.repo.upsert_priority_item(
                            ...
                            min_for_base_crates=int(row["Min For Base (crates)"])
